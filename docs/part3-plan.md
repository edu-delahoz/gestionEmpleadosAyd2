...
- Crear helpers en `lib`:
  - `lib/data/masters.ts`: funciones `listMasters`, `createMaster`, `updateBalance`.
  - `lib/data/movements.ts`: `listMovementsByMaster`, `createMovement`.
- Añadir etiquetas/`revalidatePath` si se usan Server Actions.
- Endpoint/API propuesto:
  - `app/api/masters/route.ts` → GET/POST para maestros.
  - `app/api/movements/route.ts` → GET (by master) y POST.

### 2. CRUD de Maestros (Prompt 2)
- Página destino: `app/dashboard/hr/masters/page.tsx` (nueva) o reutilizar `.../hr/employees`.
- Componentes:
  - Tabla con nombre, departamento, saldo inicial/actual, creador.
  - Botón “Nuevo maestro” (solo roles `admin` y `hr`).
  - Modal con formulario → `POST /api/masters`.
...
- UI:
  - Dropdown para elegir maestro (cargar lista real).
...
- Añadir mensajes cuando no existan maestros/movimientos.
...
1. **Prompt 1:** “Implementa helpers y endpoints `/api/masters` y `/api/movements` conectados a Prisma/Supabase. Reglas: solo HR/Admin crean maestros; todos los roles pueden crear movimientos.”
2. **Prompt 2:** “Crea la página `app/dashboard/hr/masters/page.tsx` consumiendo los endpoints, con tabla, filtros y modal ‘Nuevo maestro’ (solo HR/Admin).”
3. **Prompt 3:** “Crea la página `app/dashboard/hr/transactions/page.tsx` con dropdown maestro, tabla de movimientos, modal ‘Agregar movimiento’ y gráfica con datos reales.”
