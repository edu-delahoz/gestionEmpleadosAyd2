...
export default function HRDashboardPage() {
  const { data, loading, error, refresh } = useWorkforceSummary("hr")
      ...
      <div className="space-y-6">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Monitorea el talento actual, posiciones abiertas y solicitudes pendientes</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Incorporaciones recientes en los últimos 90 días</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {!data && loading && <p className="text-sm text-muted-foreground">Cargando colaboradores...</p>}
              {data && data.recentHires.length === 0 && (
                <p className="text-sm text-muted-foreground">Aún no hay nuevas contrataciones en el periodo.</p>
              )}
                ...
                <div key={hire.id} className="flex items-center justify-between border-b last:border-b-0 pb-3 last:pb-0">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {hire.position ?? "Posición no asignada"} · {hire.department ?? "Sin departamento"}
                    </p>
                  </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Distribución actual de empleados activos</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {!data && loading && <p className="text-sm text-muted-foreground">Calculando distribución...</p>}
              {data && data.departments.length === 0 && (
                <p className="text-sm text-muted-foreground">No hay departamentos con empleados activos.</p>
              )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Vacantes activas y su prioridad</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
          ...
          <Card>
            <CardHeader className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <CardDescription>Prioriza las atenciones críticas hoy</CardDescription>
              </div>
              <Button variant="outline" size="sm" asChild>
                <Link href="/dashboard/hr/requests">Ver solicitudes</Link>
              </Button>
            </CardHeader>
            <CardContent className="space-y-4">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Colaboradores disponibles para asignaciones rápidas</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {!data && loading && <p className="text-sm text-muted-foreground">Obteniendo lista de empleados...</p>}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Tareas inmediatas para el equipo de RH</CardDescription>
          </CardHeader>
          <CardContent>
